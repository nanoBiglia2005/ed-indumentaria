...
function LoginPage() {
  ...
  return (
    <div className='flex h-screen w-screen items-center justify-center bg-neutro-50'>
      <div className='flex flex-col items-center gap-6 px-10 py-12 rounded bg-white border border-neutro-200'>
        {error && (
        ...
        <form method='POST' action='/auth/signin/google'>
          ...
          <button
            type='submit'
            disabled={!csrfToken}
            className='flex items-center cursor-pointer text-xl gap-3 px-5 py-3 rounded border border-neutro-200 font-semibold text-sm text-neutro-600 transition-colors hover:bg-neutro-100 disabled:opacity-50'
          >
