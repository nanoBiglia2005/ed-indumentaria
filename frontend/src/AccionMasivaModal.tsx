...
export default function AccionMasivaModal({
  ...
  // Cuenta regresiva: recien se puede confirmar la accion pasados
  ...
  const handleConfirmar = async () => {
    if (segundosRestantes > 0 || isLoading) return;
            ...
            >
              <Dialog.Panel className='w-full max-w-sm transform overflow-hidden rounded-lg bg-white p-6 text-left align-middle shadow-xl transition-all'>
                <Dialog.Title as='h3' className='text-lg font-medium leading-6 text-gray-900 mb-4'>
                  {esEliminar ? 'Eliminar Artículos' : 'Imprimir Artículos'}
                </Dialog.Title>

                {error && (
                  <div className='mb-4 p-3 bg-red-100 border border-red-400 text-red-700 rounded flex flex-col'>
                    <span>{esEliminar ? 'Error al eliminar los articulos' : 'Error al imprimir los articulos'}</span>
